...
export class MoveCopy {

    outerLoop: Map<Direction, Function> = new Map<Direction, Function>([
        [Direction.UP, function (innerloop) {
            for (let j = 0; j <= MAX_INDEX; j++) {
                innerloop();
            }
        }],
        [Direction.DOWN, function (innerloop) {
            for (let j = 0; j <= MAX_INDEX; j++) {
                innerloop();
            }
        }],
        [Direction.LEFT, function (innerloop) {
            for (let i = 0; i <= MAX_INDEX; i++) {
                innerloop();
            }
        }],
        [Direction.RIGHT, function (innerloop) {
    ...
    public static up(inputGame: Game): MoveResult {
        ...
        MoveCopy.executeAfterMoveOperations(result);
        return result;
    }
